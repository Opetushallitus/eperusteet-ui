...
const lisaaTutkinnonosaNimiKoodistoon = async () => {
  ...
  try {
    if (_.isEmpty(_.get(store.value.data.tutkinnonOsa.nimi, 'fi')) || _.isEmpty(_.get(store.value.data.tutkinnonOsa.nimi, 'sv'))) {
      $fail($t('koodistoon-vienti-epaonnistui') as string, $t('tutkinnon-osan-nimi-koodisto-vienti-vaadittu-kieli-virhe') as string);
    }
    else {
      const koodi = (await Koodisto.lisaaUusiKoodi('tutkinnonosat', store.value.data.tutkinnonOsa.nimi)).data as any;
