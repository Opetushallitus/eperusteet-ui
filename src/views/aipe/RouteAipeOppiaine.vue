      ...
      <b-row>
        <b-col cols="11">
          ...
          <template v-if="!data['_oppiaine'] && oppiaineId && (!data.kurssit || data.kurssit.length == 0)">
            ...
            <ep-button variant="outline-primary" icon="plussa" @click="lisaaOppimaara" v-if="!isEditing">
              {{ $t('lisaa-oppimaara') }}
            </ep-button>
          </template>
          ...
          <template v-if="oppiaineId && (!data.oppimaarat || data.oppimaarat.length == 0)">
            ...
            <ep-button variant="outline-primary" icon="plussa" @click="lisaaKurssi" v-if="!isEditing">
              {{ $t('lisaa-kurssi') }}
            </ep-button>
          </template>
