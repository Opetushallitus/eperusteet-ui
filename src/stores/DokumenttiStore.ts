...
export class DokumenttiStore {
  ...
  constructor(private peruste: PerusteDto, private suoritustapa: string) {
  }
  ...
  async haePdf(kieli: Kieli) {
    if (this.peruste && this.suoritustapa) {
      this.state.dokumentti = (await Dokumentit.getLatestDokumentti((this.peruste.id as number), kieli.toString(), this.suoritustapa, '')).data;
      this.setHref();
    }
  }
  ...
  async luoPdf(kieli: Kieli) {
    ...
    this.state.dokumentti = (await Dokumentit.createDokumentti((this.peruste.id as number), kieli.toString(), this.suoritustapa, '')).data;
